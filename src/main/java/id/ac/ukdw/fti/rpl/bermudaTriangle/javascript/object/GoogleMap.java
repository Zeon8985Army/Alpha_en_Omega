 ...
 */
package id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.object;

import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.JavascriptObject;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.JavascriptRuntime;

import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.EventHandlers;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.GFXEventHandler;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.MapStateEventType;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.MouseEventHandler;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.StateEventHandler;
import id.ac.ukdw.fti.rpl.bermudaTriangle.javascript.event.UIEventHandler;
...
public class GoogleMap extends JavascriptObject {
     ...
     * bounds are reset after the zoom event is fired, which can cause unexpected
     * results.
     *
     * @return
     */
     ...
     * assuming you can store this and use it to later restore the view will give
     * incorrect results. Calling map.fitBounds(map.getBounds()); will result in the
     * map gradually zooming outward.
     * <p>
     *
     * @param bounds
     */
    ...
    /**
     * Returns the screen point for the provided LatLong. Note: Unexpected results
     * can be obtained if this method is called as a result of a zoom change, as the
     * zoom event is fired before the bounds are updated, and bounds need to be used
    ...
    /**
     * Registers an event handler in the repository shared between Javascript and
     * Java.
     *
    ...
    /**
     * Adds a handler for a mouse type event and returns an object that does not
     * require interaction with the underlying Javascript API.
     * 
     ...
     * JavascriptObject obj, but we would then need to loosen up the event type and
     * either accept a String value, or fill an enum with all potential state
     * events.
     *
     * @param type Type of the event to register against.
     * @param h    Handler that will be called when the event occurs.
