...
public class LatLong extends JavascriptObject {
    ...
    public double distanceFrom(LatLong end) {
        ...
        double a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(getLatitude() * Math.PI / 180)
                * Math.cos(end.getLatitude() * Math.PI / 180) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
    ...
    public LatLong getDestinationPoint(double bearing, double distance) {
        ...
        double lat2 = Math.asin(Math.sin(lat1) * Math.cos(distance / EarthRadiusMeters)
                + Math.cos(lat1) * Math.sin(distance / EarthRadiusMeters) * Math.cos(brng));

        double lon2 = lon1 + Math.atan2(Math.sin(brng) * Math.sin(distance / EarthRadiusMeters) * Math.cos(lat1),
                Math.cos(distance / EarthRadiusMeters) - Math.sin(lat1) * Math.sin(lat2));
