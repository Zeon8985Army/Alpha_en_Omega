 ...
 * <p>
 * Instances of this class are not safe for re-use at this stage. A new one
...
public class ElevationService extends JavascriptObject {
    ...
    public void getElevationForLocations(LocationElevationRequest req, ElevationServiceCallback callback) {
        ...
        StringBuilder r = new StringBuilder(getVariableName()).append(".").append("getElevationForLocations(")
                .append(req.getVariableName()).append(", ")
                .append("function(results, status) {alert('rec:'+status);\ndocument.").append(getVariableName())
    ...
    public void getElevationAlongPath(PathElevationRequest req, ElevationServiceCallback callback) {
        ...
        StringBuilder r = new StringBuilder(getVariableName()).append(".").append("getElevationAlongPath(")
                .append(req.getVariableName()).append(", ").append("function(results, status) {document.")
                .append(getVariableName()).append(".processResponse(results, status);});");
    ...
    /**
     * Processess the Javascript response and generates the required objects that
     * are then passed back to the original callback.
     * 
     * @param results
     * @param status
     */
