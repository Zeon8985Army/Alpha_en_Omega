 ...
 */
package id.ac.ukdw.fti.rpl.bermudaTriangle.service.directions;
...
import id.ac.ukdw.fti.rpl.bermudaTriangle.service.geocoding.GeocoderUtils;
...
public class DirectionsSteps extends JavascriptObject {
    ...
    /*
     * public Polyline getPolyline(){ try { JSObject location = (JSObject)
     * getJSObject().getMember("polyline"); return new Polyline((JSObject)
     * location); } catch (Exception e) {
     * Logger.getLogger(this.getClass().getName()).log(Level.FINE, "", e); } return
     * null; }
     */

    public List<DirectionsSteps> getSteps() {
        final List<DirectionsSteps> result = new ArrayList<>();
        List<JSObject> jsLocalities = GeocoderUtils
                .getJSObjectsFromArray((JSObject) getJSObject().getMember("postcode_localities"));
    ...
    public List<LatLong> getPath() {
        ...
        List<JSObject> jsLocalities = GeocoderUtils.getJSObjectsFromArray((JSObject) getJSObject().getMember("path"));
        for (JSObject jsLocality : jsLocalities) {
